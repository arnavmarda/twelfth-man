...
router.post("/ballUpdate", (req, res) => {
    ...
    Match.findOne({ _id: matchID })
        .then((match) => {
            if (whoIsBatting === "home") {
                batsmen = match.homePlayers;
                bowlers = match.awayPlayers;
            } else {
                batsmen = match.awayPlayers;
                bowlers = match.homePlayers;
            }

            battingIDX = batsmen.indexOf(batsman);
            bowlingIDX = bowlers.indexOf(bowler);
        })
        .catch((err) => {
            return res.status(400).json({
                error: "Couldn't find a match with this ID",
            });
        });
            ...
            { _id: matchID },
            {
                ...
                $set: {
                    "homeBowling.overToUpdateIDX": {
                        $concat: ["$homeBowling.overToUpdateIDX", ballSymbol],
                    },
                },
            }
        ).catch((err) => console.log(err));
